...
Q2_plot = Q2.2 %>%
  ...
  geom_text(aes(label = percentCC), nudge_y = 70, size = 4) +
...
```{r}
ggsave("Crop_CCAcreage_2022.png", plot = Q2_plot, height = 5, width = 7, dpi = 300)
```
...
Q4_plot = Q4 %>%
  ...
  scale_y_continuous(limits= c(0,25), breaks = c(0,5,10,15,20)) +
...
```{r, fig.width = 7, fig.height = 5}
 Q5_plot = planting_date %>%
  ...
  scale_color_gradient2(midpoint = 7, mid = "steelblue3", low = "slategray1", high = "blue") +
...
Q7_plot = Q7 %>%
  ...
  geom_jitter(size=3) +
  scale_color_manual(values = c("purple1","green4")) +
  theme_bw() +
  theme(legend.position = "none",
        axis.title.x = element_blank(),
        axis.title.y = element_text(size=15),
...
Q10.3 = Q10 %>% #What was the timing of applied manure
  ...
  manure_timing == c("liquid","unknown source")
```
